...
// TestWindowedThroughputSamplerConcurrency tests that GetSampleRate is safe to call concurrently
func TestWindowedThroughputSamplerConcurrency(t *testing.T) {
	testCases := []struct {
		name   string
		config *config.WindowedThroughputSamplerConfig
	}{
		...
		{
			name: "with_update_and_lookback_frequency",
			config: &config.WindowedThroughputSamplerConfig{
				FieldList:            []string{"team.id", "service.name"},
				GoalThroughputPerSec: 100,
				UpdateFrequency:      config.Duration(2 * time.Second),
				LookbackFrequency:    config.Duration(30 * time.Second),
				MaxKeys:              75,
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			testTraces := []*types.Trace{
				createTrace("web-service", "200", "get_dashboard", "user123", 1, 4),
				createTrace("api-service", "201", "create_resource", "user456", 2, 6),
				createTrace("worker-service", "500", "process_job", "user789", 1, 2),
			}
