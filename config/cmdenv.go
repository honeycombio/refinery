...
type CmdEnv struct {
	...
	OTelLoggerAPIKey                    string            `long:"otel-logger-api-key" env:"REFINERY_OTEL_LOGGER_API_KEY" description:"API key for OTel logs if being sent to Honeycomb. Setting this value via a flag may expose credentials - it is recommended to use the env var or a configuration file."`
	...
	OTelMetricsAdditionalAttributes     map[string]string `long:"otel-metrics-additional-attributes" env:"REFINERY_OTEL_METRICS_ADDITIONAL_ATTRIBUTES" env-delim:"," description:"Additional attributes to add as resource attributes on all OpenTelemetry metrics emitted by Refinery. When supplying via a environment variable, the value should be a string of comma-separated key-value pairs. When supplying via the command line, the value should be a key value pair. If multiple key-value pairs are needed, each should be supplied via its own command line flag. The key-value pairs must use ':' as the separator."`
