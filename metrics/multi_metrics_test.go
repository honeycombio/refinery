...
func TestMultiMetricsMulti(t *testing.T) {
	// Test that we can register a separate StoreMetrics and get the same results.
	// This proves that both the StoreMetrics inside the MultiMetrics and the
	// standalone StoreMetrics are getting the same data, and that both are
	// appropriately initialized through the injection graph.
	store2 := &StoreMetrics{}

	mm, err := getAndStartMultiMetrics(store2)
	assert.NoError(t, err)
	mm.Register("updown", "updowncounter")
	mm.Register("counter", "counter")
	mm.Register("gauge", "gauge")
	...
	val, ok := mm.Get("counter")
	assert.True(t, ok)
	val2, ok := store2.Get("counter")
	assert.True(t, ok)
	assert.Equal(t, val, val2)

	val, ok = mm.Get("updown")
	assert.True(t, ok)
	val2, ok = store2.Get("updown")
	assert.True(t, ok)
	assert.Equal(t, val, val2)

	val, ok = mm.Get("gauge")
	assert.True(t, ok)
	val2, ok = store2.Get("gauge")
	assert.True(t, ok)
	assert.Equal(t, val, val2)
}
